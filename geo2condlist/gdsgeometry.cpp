...
//@author:Dragon
//setCoordinate
void Point::setCoordinate(int xx, int yy, int zz ){
    x=xx;
    y=yy;
    z=zz;
}
...
{ }
    ...
    //@author:Dragon
Rectangle::Rectangle(  int x1, int x2, int y1, int y2, int z1, int z2 )
    :normal(FLAT ),x1(x1), x2(x2), y1(y1), y2(y2), z1(z1), z2(z2)
{}
...
int Rectangle::size() const{
    ...
    temp=xLen>yLen?xLen:yLen;
    return  temp>zLen?temp:zLen;
}
...
//@author:Dragon
//to check if two rectangles are intersected with each other
//Remember: This is only effective on two-cuboid problems
bool Rectangle::isIntersecting3d(const Rectangle &rect)const{
    return rect.x1<  x2 && rect.x2>x1  && rect.y1<y2 && rect.y2>y1 && rect.z1 <z2 && rect.z2>z1;

}

//@author:Dragon
//to check if a rectA ( 2d or 3d) is within a rectB (2d or 3d)
//note: the rectA can be equal to rectB
//note: rectA (2d) cannot be just at the surface of rectB
bool Rectangle::isIncludedIn( const Rectangle &rect ) const{
    if(  x1 <rect.x1 || x2 > rect.x2  || y1 <rect.y1 || y2 >rect.y2 || z1 <rect.z1 || z2 >rect.z2 ){
        return false;
    }
    //this can be equal to rect
    if(  x1 ==rect.x1 && x2 == rect.x2  && y1 ==rect.y1 && y2 ==rect.y2 && z1 ==rect.z1 && z2 ==rect.z2 ){
        return true;
    }
    //if this is 3d, then this must be within rect
    if(  x1!=x2 && y1!=y2  && z1 !=z2 ){
        return true;
    }
    //then this must be less than 3-d, maybe 2d 1d or 0d
    //this is in the surface of rect, this is not "included"
    if( (x1==x2 &&( x1== rect.x1 || x1==rect.x2)  ) || (y1==y2 &&( y1== rect.y1 || y1==rect.y2)  ) || (z1==z2 &&( z1== rect.z1 || z1==rect.z2)  )  ){
        return false;
    }
    return true;
...
that conductor (fatherConductor), this is an important property
*/
SubConductor::SubConductor():Rectangle(){
    isVia=false;
}

SubConductor::SubConductor(  int x1, int x2, int y1, int y2, int z1, int z2):Rectangle(x1,x2,y1,y2,z1,z2){
    isVia=false;
}

SubConductor::SubConductor(const Rectangle &rect):Rectangle(rect){
    isVia=false;
}

SubConductor::SubConductor(const Rectangle &rect, int fatherCondID ):Rectangle(rect){
    isVia=false;
    fatherConductorID=fatherCondID;
}

SubConductor::SubConductor( const Rectangle &rect,  int fatherCondID , bool viaOrNot ):Rectangle(rect) {
    isVia=viaOrNot;
    fatherConductorID=fatherCondID;   
}
...
// This is useful when generating Gaussian Surface
bool SubConductor::operator == (SubConductor &rhs){
    //if the two subconductors do not belong to the same conductor, then they cannot be the same  subconductor
    if(  fatherConductorID != rhs.fatherConductorID ){
        return false;
    }
    if( x1==rhs.x1 && x2==rhs.x2 && y1==rhs.y1 && y2 == rhs.y2 && z1 == rhs.z1 && z2 ==rhs.z2 ){
        return true;
    }
    return false;
...
//@author:Dragon
Conductor::Conductor()
    : nMetal(0), nVia(0), nLayer(0), layer(0), subConductorList(0),ID(0)
{ }

Conductor::Conductor(int nMetal1, int nVia1)
    : nMetal(nMetal1), nVia(nVia1), nLayer(nMetal1+nVia1), layer(nLayer, vector< RectangleList >(6)), subConductorList(0),ID(0)
{ }
Conductor::Conductor(int nMetal1, int nVia1, int indexID)
    :nMetal(nMetal1), nVia(nVia1), nLayer(nMetal1+nVia1), layer(nLayer, vector< RectangleList >(6)), subConductorList(0),ID(indexID)
{}
...
Conductor& Conductor::operator+= (  Conductor& rhs ) throw ( ConductorLayerNotCompatibleError )
{///
    ...
    //@author:Dragon
    //change the fatherConductorID of the subconductors in rhs
    
    for( SubConductorList::iterator tempSubCondIt= rhs.subConductorList.begin(); tempSubCondIt != rhs.subConductorList.end(); tempSubCondIt++  ){
        tempSubCondIt->fatherConductorID=ID;
    }  

    //@author:Dragon
    //combine two subConductorLists into one subConductorList 
    subConductorList.insert(   subConductorList.end(),  rhs.subConductorList.begin()  , rhs.subConductorList.end()     );
    ...
    : nMetal(cond.nMetal), nVia(cond.nVia), nLayer(nMetal+nVia), layer(nLayer, DirRectangleGLList(nDir, RectangleGLList()))
{
    for ( int layerIndex = 0; layerIndex < nLayer; ++layerIndex ){
        for ( unsigned dirIndex = 0; dirIndex < nDir; ++dirIndex){     //nDir=6
